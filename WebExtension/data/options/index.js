...
var restore = () => {
  ...
  document.getElementById('width').value = localStorage.getItem('width') || 500;
}
...
document.getElementById('save').addEventListener('click', () => {
  chrome.storage.local.set(cleanup({
  ...
  const num = Number(document.getElementById('width').value);
  localStorage.setItem('width', Math.max(Math.min(num, 750), 500));
});
